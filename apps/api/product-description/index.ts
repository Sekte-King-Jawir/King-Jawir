...
export const productDescriptionRoutes = new Elysia({ prefix: '/api/product-description' }).post(
  ...
  async ({ body }) => {
    try {
      ...
      logger.error({
        msg: 'Product description generation error',
        error: error instanceof Error ? error.message : 'Unknown',
      })
      return errorResponse(
        error instanceof Error ? error.message : 'Failed to generate product description',
  ...
  {
    body: t.Object({
      productInput: t.String({
        minLength: 1,
        maxLength: 500,
        description: 'Input describing the product',
      }),
    }),
    detail: {
      tags: ['AI Tools'],
      ...
      description:
        'Generate structured product description with short/long descriptions, bullets, and SEO keywords using AI. Response `data` contains `short`, `long`, `bullets`, and `seoKeywords`.',
    },
  }
)
