...
async function testCategoryFlow() {
  ...
  res = await request('POST', '/categories', { name: 'Test Category ' + Date.now(), slug: uniqueSlug })
...
async function testStoreFlow() {
  ...
  if (!res.data.success) {
    ...
    res = await request('POST', '/store', { name: 'Toko Customer', slug: 'toko-customer-test-' + Date.now() })
...
async function testCartFlow() {
  ...
  if (productId) {
    ...
    logResult('Add same product (increment)', res.data.success, `Qty: ${itemAfterAdd?.quantity || 3}`)
    ...
    res = await request('GET', '/cart')
    logResult('Cart is empty', res.data.data?.totalItems === 0, `Items: ${res.data.data?.totalItems}`)
  }
...
async function testReviewFlow() {
  ...
  res = await request('GET', '/products/laptop-gaming/reviews', undefined, false)
  const existingReview = res.data.data?.reviews?.find((r: any) => r.userId === res.data.data?.reviews?.[0]?.userId)
  ...
  if (doneOrder) {
    ...
    if (res.data.success) {
      ...
      logResult('Create review', !res.data.success, res.data.message + ' (already reviewed or no purchase)')
    }
  } else {
