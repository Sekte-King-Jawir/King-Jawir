...
export const googleRoute = new Elysia()
  ...
  .get('/google', async ({ redirect }) => {
    ...
    detail: {
      ...
      description: 'Redirects to Google login page. After login, Google will redirect back to /auth/google/callback.'
    }
  })
  // Google OAuth callback
  .get('/google/callback', async ({ query, set, cookie, jwtAccess, jwtRefresh, redirect }) => {
    ...
    detail: {
      ...
      description: 'Callback URL for Google OAuth. Handles the authorization code and creates/logs in user.'
    }
  })
