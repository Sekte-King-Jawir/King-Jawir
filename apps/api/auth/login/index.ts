...
export const loginRoute = new Elysia()
  ...
  .post(
    '/login',
    async ({ body, set, cookie, jwtAccess, jwtRefresh }) => {
      const result = await loginController.handle(body.email, body.password, jwtAccess, jwtRefresh)
    ...
    },
    {
      ...
      detail: {
        ...
        summary: 'Login user',
        description:
          'Login with email and password. Access and refresh tokens are set as httpOnly cookies.',
      },
    }
  )
