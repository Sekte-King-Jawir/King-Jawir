...
export const scrapeRoutes = new Elysia({ prefix: '/scrape' })
  .get('/shopee', async ({ query }) => {
    ...
    query: t.Object({
      ...
      sortBy: t.Optional(t.Union([t.Literal('cheapest'), t.Literal('highest_rating'), t.Literal('best_selling')])),
  ...
  .get('/top-rated', async ({ query }) => {
    ...
    detail: {
      ...
      description: 'Get the 10 highest rated products from all platforms with optional maximum price',
      parameters: [
        {
          name: 'q',
          in: 'query',
