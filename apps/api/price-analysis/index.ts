...
export const priceAnalysisRoutes = new Elysia({ prefix: '/api/price-analysis' })
  .get(
    '/',
    async ({ query }) => {
      try {
    ...
    },
    {
      query: t.Object({
        ...
        limit: t.Optional(t.Number({ minimum: 1, maximum: 50, description: 'Number of products to analyze (default: 10)' })),
        userPrice: t.Optional(t.Number({ minimum: 0, description: 'Optional: User\'s intended selling price for comparison' })),
      }),
      detail: {
        tags: ['Price Analysis'],
        summary: 'Analyze product prices from Tokopedia',
        description: 'Fetch product prices from Tokopedia and analyze them using AI to provide pricing recommendations and market insights',
      },
    }
  )
